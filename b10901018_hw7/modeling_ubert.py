...
class UbertDataModel(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_data, shuffle=True, batch_size=self.batchsize, pin_memory=True)

    def val_dataloader(self):
        return DataLoader(self.valid_data, shuffle=False, batch_size=self.batchsize, pin_memory=True)
...
class UbertLitModel(pl.LightningModule):
    ...
    def setup(self, stage) -> None:
        if stage == 'fit':
            num_devices = self.trainer.num_devices if self.trainer.num_devices is not None else 0
            self.total_step = int(self.trainer.max_epochs * self.num_data /
                                  (max(1, num_devices) * self.trainer.accumulate_grad_batches))
    ...
    def comput_metrix_span(self, logits, labels):
        ...
        y_pred = logits.contiguous().view(size=(-1,))
        y_true = labels.contiguous().view(size=(-1,))
...
class UbertPipelines:
    ...
    def __init__(self, args):
        ...
        self.trainer = pl.Trainer(precision=args.precision, accumulate_grad_batches=args.accumulate_grad_batches).from_argparse_args(args,
                                                              logger=self.logger,
